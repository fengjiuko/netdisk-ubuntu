...
void Login::regist()
{
    ...
    if (email.isEmpty() || passwd.isEmpty())
    {
        QMessageBox::critical(this, "注册", "邮箱和密码不能为空！");
        return;
    }

    // 发送注册请求
    MsgUnit* munit = MsgTools::generateRegistRequest(email, passwd);
    socket->write((char*)munit, munit->totalLen);
    free(munit);
    munit = nullptr;
    ...
    connect(&timer, &QTimer::timeout, [&loop, &connection, this](){
        disconnect(connection);
        loop.quit();
        QMessageBox::critical(this, "注册", "Timeout! " + socket->errorString());
        qDebug() << socket->state();
    ...
    if (isGetRespond)
    {
        ...
        if (respond.left(7) == "success")
        {
            QMessageBox::information(this, "注册", "注册成功！请使用新账号登录。");
            // 清空输入框
            ui->userName->clear();
            ui->passwd->clear();
        }
        else
        {
            QString statusCode = MsgTools::getRow(munit, 1);
            if (statusCode.length() <= 7)
                QMessageBox::critical(this, "注册", "通信错误");
            else
                QMessageBox::critical(this, "注册", getStatusCodeString(statusCode.mid(7)));
        }
